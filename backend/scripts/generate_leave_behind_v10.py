"""
GL-WEB-LB-010 — Leave-Behind Flyer (v10)

Identical to v9 except two targeted additions:
  1. Cost-of-inaction stat row above "SERVICES & STARTING PRICES"
  2. Explicit privacy / written-permission statement in the footer

Run once:  python3 /app/backend/scripts/generate_leave_behind_v10.py
...
OUT_PATH = os.path.join(OUT_DIR, "GigLine_LeaveBehind_v10.pdf")
os.makedirs(OUT_DIR, exist_ok=True)

QR_TARGET = (
    "https://giglinecompliance.com/walkthrough"
    "?utm_source=doorknock&utm_medium=doorknock&utm_campaign=leave-behind-v10"
)
...
def build():
    ...
    doc = SimpleDocTemplate(
        ...
        title="GigLine Safety & Compliance — Leave-Behind v10",
    ...
    COI_GOLD = HexColor("#C9A84C")

    def coi_cell(value: str, label: str):
        return [
            Paragraph(
                f'<font name="Helvetica-Bold" color="#C9A84C" size="20">{value}</font>',
                ParagraphStyle("coi_val", fontName="Helvetica-Bold", fontSize=20, leading=22,
                               textColor=COI_GOLD, alignment=TA_CENTER, spaceAfter=2),
            ),
            Paragraph(
                f'<font name="Helvetica" color="#FFFFFF" size="9">{label}</font>',
                ParagraphStyle("coi_lbl", fontName="Helvetica", fontSize=9, leading=11,
    ...
    story.append(Paragraph(
        ...
        ParagraphStyle(
            ...
            backColor=COI_NAVY, borderPadding=(4, 8, 6, 8),
        ),
    ))
    story.append(Spacer(1, 8))
    ...
    # ────────────────────────────────────────────────
    #  FOOTER — credentials byline + privacy + fine print
    # ────────────────────────────────────────────────
    story.append(Paragraph(
    ...
    story.append(Paragraph(
        "All engagements are private &mdash; findings are never shared, published, or referenced "
        "without your written permission.",
        ParagraphStyle("privacy", fontName="Helvetica-Oblique", fontSize=8.5, leading=11,
                       textColor=CHARCOAL, alignment=TA_CENTER, spaceAfter=3),
    ))
    story.append(Paragraph(
