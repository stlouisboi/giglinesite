        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 text-sm border border-gray-200 rounded focus:outline-none focus:border-[#2A52A0] w-56"
              data-testid="lead-search-input"
            />
          </div>
          <button
            onClick={() => setAddOpen(true)}
            className="inline-flex items-center gap-1 bg-[#2A52A0] hover:bg-[#1F3F80] text-white text-sm font-semibold px-3 py-1.5 rounded transition-colors"
                ...
                <tr key={lead.id} className="border-b border-gray-100 hover:bg-gray-50 cursor-pointer" onClick={() => { setDrawer(lead); setEditForm({}); setEditOpen(false); }} data-testid={`lead-row-${lead.id}`}>
                  ...
                  <td className="px-3 py-3 text-right whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      className="text-gray-400 hover:text-[#2A52A0] p-1.5 inline-block"
                      title="Edit lead"
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1">
                  {drawer.phone && <a href={`tel:${drawer.phone}`} className="text-xs text-[#2A52A0] hover:underline inline-flex items-center gap-1"><Phone size={12} /> Call</a>}
                  {drawer.email && <a href={`mailto:${drawer.email}`} className="text-xs text-[#2A52A0] hover:underline inline-flex items-center gap-1 ml-3"><Mail size={12} /> Email</a>}
                </div>
              </div>
                  ...
                  {['name', 'company', 'phone', 'email', 'service'].map((f) => (
                    <div key={f}>
                      ...
                      <input
                        ...
                        className="w-full px-3 py-1.5 text-sm border border-gray-200 rounded focus:outline-none focus:border-[#2A52A0]"
                        data-testid={`edit-field-${f}`}
                      />
                    </div>
                  ))}
                  <div className="flex gap-2 pt-1">
                    <button onClick={saveEdit} className="bg-[#2A52A0] hover:bg-[#1F3F80] text-white text-sm font-semibold px-3 py-1.5 rounded inline-flex items-center gap-1" data-testid="save-edit-btn">
                      <Save size={13} /> Save
                    </button>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <textarea
                    ...
                    className="flex-1 px-3 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-[#2A52A0] resize-none"
                    data-testid="note-input"
                  />
                  <button
                    onClick={addNote}
                    disabled={savingNote || !noteText.trim()}
                    className="bg-[#2A52A0] hover:bg-[#1F3F80] disabled:opacity-40 text-white text-xs font-semibold px-3 rounded self-stretch"
      ...
      <form onSubmit={submit} onClick={(e) => e.stopPropagation()} className="relative bg-white rounded-lg shadow-2xl w-full max-w-md p-6 space-y-3">
        ...
        {['name', 'company', 'phone', 'email'].map((f) => (
          <div key={f}>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-[#2A52A0]"
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-3 py-2 text-sm border border-gray-200 rounded focus:outline-none focus:border-[#2A52A0] bg-white"
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" disabled={saving || !form.name.trim()} className="bg-[#2A52A0] hover:bg-[#1F3F80] disabled:opacity-40 text-white text-sm font-semibold px-4 py-2 rounded" data-testid="submit-add-lead">
