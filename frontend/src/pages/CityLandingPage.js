...
const CityLandingPage = () => {
  ...
  const priceStart = data.priceStart || 1300;
  const priceRangeTop = 2100;
  const priceStartLabel = data.travelNote ? `$1,300 + travel fee` : `$${priceStart.toLocaleString()}`;
        ...
        <div className="container max-w-2xl text-center">
          ...
          {data.travelNote && (
            <p className="text-white/50 mb-8 text-sm">
              {data.name} is {data.distance}. A travel fee applies in addition to the base walkthrough price , quoted before scheduling.
            </p>
          )}
