          ...
          <div className="space-y-4 mb-12">
            {files.map((file) => (
              <a
                ...
                className="flex items-center justify-between p-5 bg-[#F9F8F6] border border-[#102133]/10 rounded hover:border-[#1a6fc4]/40 transition-colors group"
                data-testid={`download-${file.filename.split('.')[0]}`}
              >
                <div>
                  <p className="font-medium text-[#102133] text-sm group-hover:text-[#1a6fc4] transition-colors">
                    {file.label}
                  </p>
                  <p className="text-xs text-[#102133]/50 mt-1">{file.desc} — PDF</p>
                </div>
                <Download size={20} className="text-[#102133]/40 group-hover:text-[#1a6fc4] transition-colors flex-shrink-0" />
              </a>
            ))}
          </div>
