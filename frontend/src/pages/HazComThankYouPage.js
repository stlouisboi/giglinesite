    ...
    return (
      <main className="min-h-[60vh] flex items-center justify-center" data-testid="hazcom-thankyou-loading">
        <div className="text-center">
          <Loader size={32} className="animate-spin text-[#C9A84C] mx-auto mb-4" />
      ...
      <section className="bg-[#1C2B2B] text-white py-12 md:py-16">
        <div className="container max-w-3xl">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-xs font-semibold tracking-widest text-[#C9A84C] uppercase" style={{ fontFamily: "'JetBrains Mono', monospace" }}>
              PURCHASE COMPLETE
            </p>
          </div>
          <h1
          ...
          <div className="space-y-4 mb-12">
            {files.map((file) => (
              <a
                ...
                className="flex items-center justify-between p-5 bg-[#F9F8F6] border border-[#1C2B2B]/10 rounded hover:border-[#C9A84C]/40 transition-colors group"
                data-testid={`download-${file.filename.split('.')[0]}`}
              >
                <div>
                  <p className="font-medium text-[#1C2B2B] text-sm group-hover:text-[#C9A84C] transition-colors">
                    {file.label}
                  </p>
                  <p className="text-xs text-[#1C2B2B]/50 mt-1">{file.desc} — PDF</p>
                </div>
                <Download size={20} className="text-[#1C2B2B]/40 group-hover:text-[#C9A84C] transition-colors flex-shrink-0" />
              </a>
            ))}
          </div>
            ...
            <ol className="space-y-3">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-[#1C2B2B]/70 text-sm">
                  <span className="bg-[#C9A84C] text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                    {i + 1}
                  </span>
                  {step}
                </li>
