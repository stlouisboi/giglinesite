...
## Recent Changelog
- 2026-02 — **🎨 Brand rebrand: Navy is now the primary dark color (sitewide)** — Per user direction, the canonical brand dark shifted from `#1C2B2B` (deep teal/charcoal — read as "green" on some displays) to `#2A52A0` (navy). Sweep covered 134+ instances across 20+ files: all `bg-[#1C2B2B]` → `bg-[#2A52A0]`, all `hover:bg-[#1c2e44]` → `hover:bg-[#1F3F80]`, all `border-[#1C2B2B]` → `border-[#2A52A0]`, all inline `background:/backgroundColor:` styles, gradient `from-[#1C2B2B]`/`to-[#1C2B2B]` stops, `const NAVY = '#1C2B2B'` constants in 3 page files, ServicesPage pricing ternary, SampleReportSection report header, mobile browser `theme-color` meta, and CSS variables (`--primary`, `--navy`, `--navy-light`) in `src/index.css`. `text-[#1C2B2B]` (text uses) intentionally kept — reads fine as dark text on white and avoids body-copy fatigue from over-saturated navy. Confirmed clean: HomePage, Services, About, Safety Walkthrough, Case Study all rendering navy with zero residual teal backgrounds.
